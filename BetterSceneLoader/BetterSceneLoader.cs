...
{
    class BetterSceneLoader : MonoBehaviour
    {
        static string scenePath = Environment.CurrentDirectory + "/UserData/Studio/BetterSceneLoader/";
        ...
        void Start()
        {
            var loadbutton = GameObject.Find("StudioScene/Canvas Main Menu/04_System/Viewport/Content/Load");
            if(loadbutton)
            {
                ...
                if(button)
                {
                    ...
                    button.onClick.AddListener(() => UISystem.gameObject.SetActive(!UISystem.gameObject.activeSelf));
                }
            }

            var savebutton = GameObject.Find("StudioScene/Canvas Main Menu/04_System/Viewport/Content/Save");
            if(savebutton)
            {
                ...
                {
                    button.onClick = new Button.ButtonClickedEvent();
                    button.onClick.AddListener(() => SaveScene());
                }
            }
        }
        ...
        bool LoadSettings()
        {
            columnCount = int.Parse(BepInEx.Config.GetEntry("ColumnCount", "3", "BetterSceneLoader"));
            scrollSensitivity = float.Parse(BepInEx.Config.GetEntry("ScrollSensitivity", "3", "BetterSceneLoader"));
            autoClose = bool.Parse(BepInEx.Config.GetEntry("AutoClose", "true", "BetterSceneLoader"));
            smallWindow = bool.Parse(BepInEx.Config.GetEntry("SmallWindow", "true", "BetterSceneLoader"));
